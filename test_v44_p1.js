...
const DataAPI = {
  ...
  /** v4.1 大盘情绪：沪深创三大指数涨跌幅（push2 ulist接口，浏览器可用；失败返回null降级为活跃股广度） */
  async fetchMarketBreadth() {
    ...
    try {
      const url = 'https://push2.eastmoney.com/api/qt/ulist.np/get?fltt=2&invt=2'
        + '&fields=f2,f3,f12,f14&secids=1.000001,0.399001,0.399006';
      const ctrl = new AbortController();
      ...
      const idx = data.data.diff.map(d => ({ name: d.f14, changePct: parseFloat(d.f3) || 0 }));
      const avg = idx.reduce((sum, x) => sum + x.changePct, 0) / idx.length;
      const payload = { indexes: idx, avgChange: avg };
...
const NextDayPrediction = {
  ...
  async scan() {
    ...
    try {
      ...
      const mkt = this._marketAdjust(breadth, upRatio);
  ...
  _renderList(snapshot) {
    ...
    html += '</div>';
    html += '<div style="font-size:11px;color:var(--text-muted);margin-top:10px;line-height:1.6">六维模型：资金持续性20 · 量价动能22 · 趋势技术20 · 位置动量18 · 板块共振12 · 龙虎榜催化8，大盘情绪全局±10。点击个股进入详细分析。排名仅为基于公开数据的短线概率统计，不构成投资建议；不预测具体涨幅。历史快照保存在本机，最多留存30个交易日。</div>';
    body.innerHTML = html;
  },
...
// === 测试1：健康上涨股 ===
console.log('\n=== 测试1：健康上涨股（多头排列+资金加速流入） ===');
const kl1 = genKlines(60, 20, 'up');
const s1 = {
  price: kl1[kl1.length-1].close,
  changePct: 3.5,
  amplitude: 5.2,
  turnover: 5,
  mainFlow: 80000000,
...
const r1 = NextDayPrediction._v2Score(s1);
console.log('总分:', r1.score.toFixed(1));
console.log('六维:', JSON.stringify(r1.dims));
console.log('因子:', r1.factors);
console.log('风险数:', r1.risks.length);
r1.risks.slice(0,3).forEach(r => console.log('  ⚠️ ' + r));

// === 测试2：加速暴涨龙头 ===
console.log('\n=== 测试2：暴涨龙头（套牢盘极少+龙虎榜） ===');
const kl2 = genKlines(60, 10, 'up');
for (let i = 50; i < 60; i++) {
  const prevClose = i > 0 ? kl2[i-1].close : kl2[i].open;
  kl2[i].close = prevClose * 1.06;
  kl2[i].high = kl2[i].close * 1.03;
  kl2[i].low = prevClose * 1.01;
  kl2[i].open = prevClose * 1.02;
  kl2[i].volume = 25000000;
}
const s2 = {
  price: kl2[59].close,
  changePct: 6.5,
  ...
  _dt: { netBuy: 50000000, seatInfo: '机构净买入', times: 2 },
  _indScore: 20
};
const r2 = NextDayPrediction._v2Score(s2);
console.log('总分:', r2.score.toFixed(1));
console.log('六维:', JSON.stringify(r2.dims));
console.log('因子:', r2.factors);
r2.risks.slice(0,4).forEach(r => console.log('  ⚠️ ' + r));

// === 测试3：深套下跌股 ===
console.log('\n=== 测试3：下跌趋势深套股（高套牢盘+资金流出） ===');
const kl3 = genKlines(60, 30, 'down');
const s3 = {
  price: kl3[kl3.length-1].close,
  changePct: 1.5,
  amplitude: 3,
  turnover: 2,
  mainFlow: 5000000,
...
const r3 = NextDayPrediction._v2Score(s3);
console.log('总分:', r3.score.toFixed(1));
console.log('六维:', JSON.stringify(r3.dims));
console.log('因子:', r3.factors);
r3.risks.slice(0,4).forEach(r => console.log('  ⚠️ ' + r));

// === 测试4：资金减速流入 ===
console.log('\n=== 测试4：资金减速流入（加速度因子测试） ===');
const kl4 = genKlines(40, 15, 'up');
const s4 = {
  price: kl4[kl4.length-1].close,
  changePct: 2.5,
  amplitude: 4,
  turnover: 6,
  mainFlow: 40000000,
...
const r4 = NextDayPrediction._v2Score(s4);
console.log('总分:', r4.score.toFixed(1));
console.log('资金维度:', r4.dims.capital.toFixed(1));
console.log('因子:', r4.factors);
r4.risks.slice(0,3).forEach(r => console.log('  ⚠️ ' + r));

// === 总结 ===
console.log('\n📊 四组对比汇总:');
console.log('  健康上涨股:', r1.score.toFixed(1).padStart(5), '分');
console.log('  暴涨龙头  :', r2.score.toFixed(1).padStart(5), '分');
console.log('  深套反弹  :', r3.score.toFixed(1).padStart(5), '分');
console.log('  资金减速  :', r4.score.toFixed(1).padStart(5), '分');
console.log('  分差(最高-最低):', (Math.max(r1.score,r2.score,r3.score,r4.score) - Math.min(r1.score,r2.score,r3.score,r4.score)).toFixed(1), '分');
