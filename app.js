...
const NextDayPrediction = {
  ...
  _renderList(snapshot) {
    ...
    html += '</div>';
    html += '<div style="font-size:11px;color:var(--text-muted);margin-top:10px;line-height:1.6">六维模型v4.4 P5：资金加速度20 · 量价筹码22 · 趋势技术20 · 位置动量18 · 板块共振12(+持续性2) · 龙虎榜催化8，大盘情绪全局±10（指数+涨跌比+涨停数+量能）。点击个股进入详细分析。排名仅为基于公开数据的短线概率统计，不构成投资建议；不预测具体涨幅。历史快照保存在本机，最多留存30个交易日。点下方「历史验证台账」查看胜率统计、因子IC分析及单股回溯。</div>';
    body.innerHTML = html;
  },
