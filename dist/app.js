...
const NextDayPrediction = {
  ...
  _v2Score(s) {
    ...
    if (cf && cf.flows && cf.flows.length >= 2) {
      const f0 = cf.flows[cf.flows.length - 1].main;
      const fPrev = cf.flows[cf.flows.length - 2].main;
      if (f0 > 0 && fPrev > 0) { f1 += 7; factors.push('主力连续2日净流入'); }
      else if (f0 > 0) f1 += 4;
      else if (f0 < 0 && fPrev > 0) { f1 += 1; risks.push('前一日主力流入今日转为流出，资金接力存疑'); }
      else f1 += 0;
    } else { f1 += 3; } // 数据缺失给中性分
    ...
    if (volRatio >= 1.5 && volRatio <= 3) { f2 += 8; factors.push('温和放量' + volRatio.toFixed(1) + '倍'); }
    else if (volRatio > 3 && volRatio <= 5) { f2 += 5; risks.push('今日放量' + volRatio.toFixed(1) + '倍偏大，若股价滞涨需防主力出货'); }
    else if (volRatio > 5) { f2 += 3; risks.push('巨量' + volRatio.toFixed(1) + '倍成交，多空分歧剧烈，次日方向不确定'); }
    else if (volRatio >= 0.8 && volRatio < 1.5) f2 += 5;
    else if (volRatio >= 0.5) f2 += 2;
    ...
    if (closePos >= 0.8) { f2 += 7; factors.push('收在全天高位'); }
    else if (closePos >= 0.5) f2 += 5;
